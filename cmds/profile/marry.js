...
export default {
  ...
  run: async ({ msg, sock, usedPrefix, command, text }) => {
    ...
    const existing = proposals.get(proposee);
    if (existing && existing.proposee === proposer) {
      ...
      const prev = proposals.get(proposer);
      if (prev) clearTimeout(prev.timeout);
      const timeout = setTimeout(() => proposals.delete(proposer), 120000);
      if (typeof timeout?.unref === 'function') timeout.unref();
      proposals.set(proposer, { proposee, timeout });
