...
async function getImageSearchResults(query) {
  const apis = [
    { endpoint: `${global.APIs.siputzx.url}/api/s/bimg?query=${encodeURIComponent(query)}`, extractor: (res) => {
        if (!res.status || !Array.isArray(res.data)) return [];
        return res.data.map(url => ({ url, title: null, domain: null, resolution: null }));
      }
    },
    { endpoint: `${global.APIs.siputzx.url}/api/s/brave?query=${encodeURIComponent(query)}`, extractor: (res) => {
        if (!res.status || !Array.isArray(res.data?.results)) return [];
        return res.data.results.map(item => ({ url: item.imageUrl, title: item.title || null, domain: null, resolution: null }));
      }
    }
  ];
