...
const HDR = { accept: 'text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,*/*;q=0.8', 'accept-language': 'es-ES,es;q=0.9,en;q=0.8', 'cache-control': 'no-cache', pragma: 'no-cache', 'sec-fetch-dest': 'document', 'sec-fetch-mode': 'navigate', 'sec-fetch-site': 'none', 'sec-fetch-user': '?1', 'upgrade-insecure-requests': '1', 'user-agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/125.0.0.0 Safari/537.36' }
const fb = async (url, { headers = {}, timeout = 45e3, binary = false } = {}) => {
  const controller = new AbortController()
  const timer = setTimeout(() => controller.abort(), timeout)
  try {
    const res = await fetch(url, { redirect: 'follow', headers: { ...HDR, ...headers }, signal: controller.signal })
...
const unesc = s => String(s || '').replace(/\\u([0-9a-f]{4})/gi, (_, h) => String.fromCharCode(parseInt(h, 16))).replace(/\\\//g, '/')
const decodeHtml = s => String(s || '').replace(/&amp;/g, '&').replace(/&#x([0-9a-f]+);/gi, (_, h) => String.fromCodePoint(parseInt(h, 16))).replace(/&#(\d+);/g, (_, d) => String.fromCodePoint(parseInt(d, 10))).replace(/&quot;/g, '"').replace(/&#39;/g, "'").replace(/&lt;/g, '<').replace(/&gt;/g, '>').replace(/&nbsp;/g, ' ').trim()
const parseNum = v => {
...
function parseStats(h, id) {
  ...
  const pid = h.match(new RegExp(`"video":\\{"id":"${id}"[\\s\\S]{0,12000}?"post_id":"(\\d+)"`))?.[1] || h.match(/"post_id":"(\d+)"/)?.[1]
  const last = ogT?.split('|').pop()?.trim()
  const s = { reelId: id, url: reelPage(id), description: decodeHtml(ogD || (last && !/views|reproducciones|reactions|reacciones/i.test(last) ? last : null)), views: +(h.match(/"(?:play|video_view|view)_count":(\d+)/)?.[1] || '') || fromTitle(ogT, 'reproducciones|views?'), reactions: pid ? +(beforePost(h, pid, /"unified_reactors":\{"count":(\d+)/) || '') : null, comments: pid ? +(beforePost(h, pid, /"total_comment_count":(\d+)/) || '') : null, shares: pid ? parseNum(beforePost(h, pid, /"share_count_reduced":"([^"]+)"/)) : null, ownerId: h.match(new RegExp(`facebook\\.com/(\\d+)/videos/[^"']*${id}`))?.[1] }
...
function parseVideo(h, id) {
  ...
  return decodeHtml(unesc(m(/"browser_native_hd_url":"((?:\\.|[^"\\])+)"/) || m(/"browser_native_sd_url":"((?:\\.|[^"\\])+)"/) || ''))
}
...
async function getReel(text) {
  ...
  const pages = [...new Set([source, url, `https://m.facebook.com/reel/${id}`, `https://web.facebook.com/reel/${id}`, `https://www.facebook.com/watch/?v=${id}`, `https://m.facebook.com/watch/?v=${id}`])]
  if (!html || !parseVideo(html, id)) {
    ...
    for (const page of pages) {
      try {
        ...
        if (r.body && (r.status === 200 || r.body.includes(`"id":"${id}"`) || r.body.includes('browser_native_hd_url') || r.body.includes('browser_native_sd_url'))) {
          html = r.body
          break
        }
...
async function getFacebookMedia(url) {
  ...
  return { type: 'video', title: decodeHtml(target.description || 'Facebook Video'), resolution: 'HD', format: 'mp4', url: target.videoUrl, thumbnail: null, duration: null }
}
