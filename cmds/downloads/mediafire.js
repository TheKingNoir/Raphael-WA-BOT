...
export default {
  ...
  run: async ({ msg, sock, args, usedPrefix, command, text }) => {
    ...
    try {
      ...
      let jpegThumbnail2 = await getBuffer(`https://cdn.yuki-wabot.my.id/files/5txZ.jpeg`)
      let jpegThumbnail = await sharp(jpegThumbnail2).resize(300, 300).jpeg({ quality: 80 }).toBuffer()
      await sock.sendMessage(msg.chat, { document: { url: scraped.downloadLink }, mimetype: tipo, fileName: title, jpegThumbnail }, { quoted: msg })
    } catch (e) {
...
async function mediafireDl(url, timeout = 45000) {
  ...
  let downloadLinkRaw = $('#downloadButton').attr('href') || $('a#downloadButton').attr('href') || null
  if (!downloadLinkRaw) downloadLinkRaw = String(res.data || '').match(/https:\/\/download\d+\.mediafire\.com[^"']+/i)?.[0] || null
