...
export default {
  ...
  run: async ({ msg, sock, args, usedPrefix, command, groupMetadata, participants }) => {
    ...
    const ownerBots = global.owner.map(num => num + '@s.whatsapp.net');
    ...
    if (args[0] === 'num' || args[0] === 'listnum') {
      ...
      const usersToKick = allUsersWithPrefix.filter(jid => {
        ...
        if (jid === ownerGroup || ownerBots.includes(jid)) return false;
        return true;
      });
    ...
    if (args[0] === 'all') {
      const usersToKick = participants.filter(p => p.id && p.id !== botId && p.id !== ownerGroup && !ownerBots.includes(p.id) && p.admin !== 'admin' && p.admin !== 'superadmin').map(p => p.id);
    ...
    if (args[0] === 'inactive' || args[0] === 'listinactive') {
      ...
      for (const participant of participants) {
        ...
        if (!jid || jid === botId || jid === ownerGroup || ownerBots.includes(jid)) continue;
    ...
    if (msg.mentionedJid?.length > 1) {
      const targets = msg.mentionedJid.map(jid => {
        const base = jid.split('@')[0];
        const p = participants.find(p => p.id?.split('@')[0] === base || p.lid?.split('@')[0] === base);
        return p ? (p.id || jid) : null;
      }).filter(Boolean);
      const usersToKick = targets.filter(jid => jid !== botId && jid !== ownerGroup && !ownerBots.includes(jid) && participants.find(p => p.id === jid)?.admin !== 'admin' && participants.find(p => p.id === jid)?.admin !== 'superadmin');
      const noEliminados = targets.length - usersToKick.length;
      if (usersToKick.length === 0) return msg.reply('《✧》 No se puede eliminar a ninguno de los mencionados (admins, propietarios o no están en el grupo).');
      await msg.reply(`《✧》 *Eliminando ${usersToKick.length} usuario(s)*${noEliminados > 0 ? ` (${noEliminados} omitido(s))` : ''}\n> El proceso tomará unos segundos...`);
      let eliminados = 0, errores = [];
      for (const jid of usersToKick) {
        try { await sock.groupParticipantsUpdate(msg.chat, [jid], 'remove'); eliminados++; await new Promise(r => setTimeout(r, 3000)); }
        catch (e) { errores.push(`@${jid.split('@')[0]}: ${e.message}`); }
    ...
    if (ownerBots.includes(realJid))   return msg.reply('《✧》 No puedo eliminar al *propietario* del bot');
    try {
