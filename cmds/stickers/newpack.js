...
export default {
  ...
  run: async ({ msg, sock, args, usedPrefix, command }) => {
    try {
      ...
      const newPack = { id: Date.now().toString(), lastModified: Date.now().toString(), name, author: 'Raphael Wᴀʙᴏᴛ', desc: `Paquete de stickers creado por ${dev}`, stickers: [], spackpublic: 0 }
      packs.push(newPack)
