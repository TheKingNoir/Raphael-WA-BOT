...
export default {
  ...
  run: async ({ msg, sock, text }) => {
    try {
      ...
      await sock.sendMessage(msg.chat, { stickerPack: { name: packName, publisher: author?.name || author?.username || `@${name}`, description: '𝐑𝐀𝐏𝐇𝐀𝐄𝐋 𝐖𝐀-𝐁𝐎𝐓', cover, stickers: stickerResults }}, { quoted: msg });      
      await msg.react('✔️');
    } catch (e) {
      await msg.react('✖️');
