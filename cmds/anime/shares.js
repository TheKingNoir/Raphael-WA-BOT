...
const captions = {
  ...
  cook: (from, to) => from === to ? 'está cocinando algo para sí mismo.' : 'está cocinando algo delicioso para',
  facepalm: (from, to) => from === to ? 'se da un facepalm por su propio error.' : 'no puede evitar hacer un facepalm por',
  greet: (from, to) => from === to ? 'se saluda a sí mismo frente al espejo.' : 'está saludando a',
  lewd: (from, to) => from === to ? 'tiene pensamientos un poco traviesos.' : 'le lanza una mirada traviesa a',
  poke: (from, to) => from === to ? 'se da golpecitos a sí mismo sin razón.' : 'le da un golpecito a',
  psycho: (from, to) => from === to ? 'tiene un ataque de locura repentino.' : 'mira con locura a',
};
...
export default {
  ...
  run: async ({ msg, sock, usedPrefix, command }) => {
    ...
    try {
      ...
      const videos = sharesData.sfw[currentCommand];
