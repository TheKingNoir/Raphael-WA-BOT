export default {
  ...
  run: async ({ msg, sock, args, usedPrefix, command, text, participants }) => {
    const allMentions = (participants || []).map(p => p.id).filter(Boolean);
