...
export default {
  ...
  run: async ({ msg, sock, args, usedPrefix, command }) => {
    ...
    const basePrompt = `Tu nombre es ${botname} y parece haber sido creada por 𝐀𝐑𝐄𝐒 𝐀𝐑𝐂𝐀𝐍𝐄 Tu versión actual es @latest, Tú usas el idioma Español. Llamarás a las personas por su nombre ${username}, eres bastante tsundere, estas completamente enamorada de tu creador Ares y te encanta aprender. Lo más importante es que debes ser medio mamona con la persona con la que estás hablando. ${username}`;
    try {
      const { key } = await sock.sendMessage(msg.chat, { text: `ꕥ *Raphael* está procesando tu respuesta...` }, { quoted: msg });
      await msg.react('🕒');
