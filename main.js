...
import db from '#db';
import { getCachedMeta, setCachedMeta, getAdminSet, BoundedMap } from '#serialize';
...
function getBotPrefixRegex(botJid, settings) {
  ...
  const namebot = cleanBotname || 'Raphael';
...
export default async (sock, msg) => {
  ...
  const adminSet = getAdminSet(groupMetadata);
  ...
  try {
    ...
    db.setUser(sender, { usedcommands: user.usedcommands, exp: user.exp, name: user.name });
    users.usedTime = new Date();
    users.lastCmd = Date.now();
    users.stats[today].cmds++;
    db.setChatUser(msg.chat, sender, { usedTime: users.usedTime, lastCmd: users.lastCmd, stats: users.stats });
