...
const log = {
  ready: (msg) => console.log(chalk.bgMagenta.blueBright.bold(`( ❏ READY )`), chalk.gray(msg)),
  reload: (msg) => console.log(chalk.yellowBright.bold(`( ↺ RELOAD )`), chalk.white(msg)),
...
async function scan(dir) {
  ...
  for (let i = 0; i < results.length; i++) {
    if (results[i].status === 'rejected') {
      errCount++;
      log.error(`Plugin falló al cargar → ${chalk.white.bold(path.basename(files[i]))} · ${results[i].reason?.message ?? results[i].reason}`);
    }
  }
...
async function reloadFile(filePath) {
  if (!filePath.endsWith('.js')) return { ok: true };
  if (!fs.existsSync(filePath)) {
    ...
    log.reload(`Plugin eliminado → ${chalk.white(path.basename(filePath))}`);
    return { ok: true };
  }
  const mtime = fs.statSync(filePath).mtimeMs;
  if (pluginCache.get(filePath)?.mtime === mtime) return { ok: true };
  try {
    const mod = await importModule(filePath);
    registerModule(filePath, mod);
    log.reload(`Cambio detectado → ${chalk.white(path.basename(filePath))}`);
    return { ok: true };
  } catch (e) {
    log.error(`Error recargando → ${chalk.white.bold(path.basename(filePath))} · ${e?.message ?? e}`);
    return { ok: false };
  }
}
...
export async function rescan(relPaths) {
  const raiz = path.resolve(__dirname, '..');
  let total = 0, errors = 0;
  for (const rel of relPaths) {
    const filePath = path.resolve(raiz, rel);
    total++;
...
export default async function cmdsLoader() {
  ...
  const ms = Date.now() - t;
  log.ready(`${global.comandos.size}/${total} comandos cargados en ${ms < 1000 ? `${ms}ms` : `${(ms / 1000).toFixed(1)}s`}${errors > 0 ? ` · ${errors} con errores` : ''}`);
